...
test = lapply(EDM_list, function(item){
    item$ccm = determineCausality(data = item$data_std[, -c(1,2)], 
                                  dim.list = item$E, 
...
keepSignificant = function(ccm_result){
    data = data.frame(ccm_result)
...
test = lapply(test, function(item){
    item$ccm_significant = lapply(item$ccm, FUN = keepSignificant)
    
    item$ccm_most_significant = lapply(item$ccm_significant, FUN = function(ccm_results){
        ccm_results[1, ]
    })
    item$ccm_most_significant = do.call(rbind, item$ccm_most_significant)
    row.names(item$ccm_most_significant) = NULL
...
test = lapply(test, feasibleCVEmbedding)
# check if each species has feasible embeddings
lapply(test, function(item){return(item$E_feasible)})  #Sprattus sprattus
...
test = lapply(test, generateSmapData)
...
plotSmapCoeff = function(data_of_coeff, lag_of_var, species=NULL, 
    ...
    #sort data
    variables = c("Shannon.age", "Total.CPUE", "AMO", 
                  "MeanBT", "CVofBT", "MeanSST", "CVofSST")
    ...
    #variable: c('Shannon.age','Total.CPUE','AMO','Mean.BT','CV.BT')
...
lapply(test, function(item){
    ...
    for (i in 1:length(smap_model_index)){
        ...
        ggsave(paste0(wd, "output\\smap\\smap_", mode, species, i, ".png"),
