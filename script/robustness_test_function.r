...
robust = function(data, dim.cv, lags){
    n = NROW(data) # length of time series
    n.var = NCOL(data[, -1]) # number of variables
    mask = data.frame(permu(n.var, dim.cv-1)) # a mask data frame to choose variables
    ...
    coeff.mean = apply(mask, 1, FUN=function(x){
        ...
        block.m = apply(matrix(seq_along(lags)), 1, FUN=function(y){
            if (lags[y] > 0){ c(rep(NA,lags[y]), block.m[, y][-c((n-lags[y]+1):n)]) }
            else { block.m[, y] }
        }) 
        block.m = cbind(subset(data, select=c('CV.CPUE')), block.m)
